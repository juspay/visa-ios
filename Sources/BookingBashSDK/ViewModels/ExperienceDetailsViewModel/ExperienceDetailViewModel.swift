...
class ExperienceDetailViewModel: ObservableObject {
    ...
    func fetchReviewData(productCode: String, currency: String) {
        guard let url = URL(string: "https://common-servicessit.vetravel.io/api/activities/2.0/details") else { return }
        
        let requestBody = ExperienceRequest(
            product_code: productCode,
            currency: currency
        )
        NetworkManager.shared.post(url: url, body: requestBody) { (result: Result<DetailExrienceApiResponse, Error>) in
